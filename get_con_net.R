...
x <- read.table("F:/MeRIPSinglebase/metdb2/result/gene_id15.txt")
biog <- as.character(x$V1)
...
sum(is.element(biog, hint))
sum(is.element(biog, iRef))
sum(is.element(biog, multinet))
...
## get con edge
id <- unique(c(hint, iRef, multinet))
...
inde <- cbind(match(hime$V1, id), match(hime$V2, id))
